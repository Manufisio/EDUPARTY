// ─────────────────────────────────────────────────────────────────────────────
// EduParty — Lógica del juego
// Versión actualizada: fixes de tablero + funciones auxiliares + mejoras varias
// ─────────────────────────────────────────────────────────────────────────────

// ─── FUNCIONES AUXILIARES DE MOVIMIENTO DE FICHAS ───────────────────────────
// Calcula el desplazamiento relativo para cada paso del dado en el tablero.
...
//Variables de estado
var pantallaActualQuiz = false;
var pantallaActualTabu = false;
var pantallaActualDibujo = false;
var pantallaActualMimica = false;
var pantallaActualParty = false;
var pantallaActualEP = false;
...
setInterval(function(){
          ...
          else if(playTime && pantallaActualEP)
          {
              ...
              if(TotMinSec[0]>=1)
                $("#imagenAscensor").animate({top: '-='+(57/duracionJuego)+'%'},{duration: 995, easing: "linear"});
...
$(document).ready(function() {
  $("#botonQuizMP").hover(function(){ $("#DescQuizMP").css("display", "block"); }, function(){ $("#DescQuizMP").css("display", "none"); });
  $("#botonTabuMP").hover(function(){ $("#DescTabuMP").css("display", "block"); }, function(){ $("#DescTabuMP").css("display", "none"); });
  $("#botonMimicaMP").hover(function(){ $("#DescMimicaMP").css("display", "block"); }, function(){ $("#DescMimicaMP").css("display", "none"); });
  $("#botonDibujoMP").hover(function(){ $("#DescDibujoMP").css("display", "block"); }, function(){ $("#DescDibujoMP").css("display", "none"); });
  $("#botonPartyMP").hover(function(){ $("#DescPartyMP").css("display", "block"); }, function(){ $("#DescPartyMP").css("display", "none"); });
  $("#botonEPMP").hover(function(){ $("#DescEPMP").css("display", "block"); }, function(){ $("#DescEPMP").css("display", "none"); });
});
...
function PreguntaQuiz() {
    ...
    if(pantallaActualParty) {
        $("#ImagenDCPQFallo").parent().before('<div class="col-1" id="placeholder3Quiz"></div>');
...
function ContinuarQuiz() {
  ...
  puntuacionEquipos[turnoEquipo] = puntuacionEquipos[turnoEquipo] + puntosProvisionales;
  for(var i=0; i<10; i++){
...
function RepetirQuiz() { ocultarTodo(); PreguntaQuiz(); }
function PasarTurno() {
  turnoEquipo++;
  if(turnoEquipo>=numEquipos) turnoEquipo = 0;
  ocultarTodo(); PreguntaQuiz();
}
...
function PreguntaTabu() {
    ...
    if(pantallaActualParty) {
        $("#ImagenDCPQFalloTabu").parent().before('<div class="col-1" id="placeholder3Tabu"></div>');
...
function RepetirTabu() { ocultarTodo(); PreguntaTabu(); }
function PasarTurnoTabu() {
  turnoEquipo++;
  if(turnoEquipo>=numEquipos) turnoEquipo = 0;
  ocultarTodo(); PreguntaTabu();
}
...
function PreguntaMimica() {
    ...
    if(pantallaActualParty) {
        $("#ImagenDCPQFalloMimica").parent().before('<div class="col-1" id="placeholder3Mimica"></div>');
...
function RepetirMimica() { ocultarTodo(); PreguntaMimica(); }
function PasarTurnoMimica() {
  turnoEquipo++;
  if(turnoEquipo>=numEquipos) turnoEquipo = 0;
  ocultarTodo(); PreguntaMimica();
}
...
function PreguntaDibujo() {
    ...
    if(pantallaActualParty) {
        $("#ImagenDCPQFalloDibujo").parent().before('<div class="col-1" id="placeholder3Dibujo"></div>');
...
function RepetirDibujo() { ocultarTodo(); PreguntaDibujo(); }
function PasarTurnoDibujo() {
  turnoEquipo++;
  if(turnoEquipo>=numEquipos) turnoEquipo = 0;
  ocultarTodo(); PreguntaDibujo();
}
...
function PrePartyGame() {
  ...
  $("#FichaEquipo1").css("top", "74%"); $("#FichaEquipo1").css("left", "7.5%");
  $("#FichaEquipo5").css("top", "74%"); $("#FichaEquipo5").css("left", "15%");
  $("#FichaEquipo2").css("top", "74%"); $("#FichaEquipo2").css("left", "22.5%");
  $("#FichaEquipo3").css("top", "88%"); $("#FichaEquipo3").css("left", "7.5%");
  $("#FichaEquipo6").css("top", "88%"); $("#FichaEquipo6").css("left", "15%");
  $("#FichaEquipo4").css("top", "88%"); $("#FichaEquipo4").css("left", "22.5%");
  ...
  duracionJuegosParty = [
      parseInt($("#TiempoPartyQuizMin :selected").text())*60 + parseInt($("#TiempoPartyQuizSec :selected").text()),
  ...
  if($("#PartyQuizSelect select").attr('disabled') != 'disabled') {
      switch($("#prioridadPartyQuiz :selected").text()) {
          case "Alta": prioridadQuiz = 4; break;
          case "Normal": prioridadQuiz = 2; break;
          case "Baja": prioridadQuiz = 1; break;
      }
  }
  if($("#PartyTabuSelect select").attr('disabled') != 'disabled') {
      switch($("#prioridadPartyTabu :selected").text()) {
          case "Alta": prioridadTabu = 4; break;
          case "Normal": prioridadTabu = 2; break;
          case "Baja": prioridadTabu = 1; break;
      }
  }
  if($("#PartyMimicaSelect select").attr('disabled') != 'disabled') {
      switch($("#prioridadPartyMimica :selected").text()) {
          case "Alta": prioridadMimica = 4; break;
          case "Normal": prioridadMimica = 2; break;
          case "Baja": prioridadMimica = 1; break;
      }
  }
  if($("#PartyDibujoSelect select").attr('disabled') != 'disabled') {
      switch($("#prioridadPartyDibujo :selected").text()) {
          case "Alta": prioridadDibujo = 4; break;
          case "Normal": prioridadDibujo = 2; break;
          case "Baja": prioridadDibujo = 1; break;
      }
  }

  prioridadTotal = prioridadDibujo + prioridadMimica + prioridadTabu + prioridadQuiz;
  if(prioridadTotal == 0) prioridadTotal = 1; // Evita división por cero
  ...
  // FIX: Ajuste seguro de casillas sobrantes (antes había bug de variable j sin definir)
  while((casillasdeQuiz + casillasdeTabu + casillasdeMimica + casillasdeDibujo) < 12) {
      if(prioridadQuiz > 0) { casillasdeQuiz++; }
      else if(prioridadTabu > 0) { casillasdeTabu++; }
      else if(prioridadMimica > 0) { casillasdeMimica++; }
      else if(prioridadDibujo > 0) { casillasdeDibujo++; }
      else { casillasdeQuiz++; } // Fallback
  }
...
function ContinuarParty(resultado) {
    ...
    if(resultado == "PartyWin" && RondaEquipo<=2) {
      ...
      // Aviso informativo de fin de las 3 rondas
      $("#textoNotificacionParty1").text("¡Has acertado la prueba!");
      $("#textoNotificacionParty2").text("Has completado tus 3 rondas. Aunque has acertado, ahora le toca al siguiente equipo.");
...
function BajarSnake() {
  if (posicionEquiposParty[turnoEquipo] == 6) {
    ...
    $("#FichaEquipo" + (turnoEquipo+1)).animate({top: '+=8.1vw', left: '+=11.42vw'}, {
      duration: 800,
      easing: "swing",
      complete: function() { setTimeout(function(){ partyElegirPrueba(); }, 400); }
    });
...
function subirEscalera() {
  if (posicionEquiposParty[turnoEquipo] == 7) {
    posicionEquiposParty[turnoEquipo] = 9;
    setTimeout(function(){
      $("#FichaEquipo" + (turnoEquipo+1)).animate({top: '-=8.1vw', left: '+=11.42vw'}, 800);
    }, 1300);
...
function animarDado(numDadoSacado) {
  ...
  for(var k=1; k<=8; k++) {
    if(k!=8){
      setTimeout(function(){
        Alea3 = random(6)+1;
        while(Alea3 == ultimoNum) { Alea3 = random(6)+1; }
...
function PrePreEP() {
  ...
  if(TotMinSec[0]==0) alert("La duración mínima se ha establecido a 10 segundos");
  PreElevatorPitch();
}
...
function FinalizarJuego(Prueba) {
  if(Prueba == "Party") {
    ...
    $("#TextoVictoriaPrincipal").text("¡La victoria es para el Equipo " + (turnoEquipo+1) + "!");
    ...
    for(var i=1; i<=numEquipos; i++) {
      ...
      if(puntuacionMaxima==0) puntuacionMaxima=1;
